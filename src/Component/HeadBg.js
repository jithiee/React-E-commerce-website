...
const HeadBg = () => {
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', fontFamily: 'cursive' }}>
        <Button className="btn1 w-25 fw-bold" variant="outline-dark" onClick={() => navigate('/men')}>
          SHOP MEN
        </Button>
        <Button className="btn2 w-25 fw-bold" variant="outline-dark" onClick={() => navigate('/women')}>
          SHOP WOMEN
        </Button>
      </div>
      <br />
      <p style={{ color: '#F7CA18', fontFamily: 'cursive', fontWeight: 'bold', fontSize: '2.9rem', textShadow: '1px 1px 0 #fff, 0 0 9px #000' }}>
        New Arrivals
      </p>
      <br />
      <hr />
      ...
      <Container className="pb-5">
        <div className="d-flex flex-wrap justify-content-center align-items-center">
          {datas.map((item) => (
            <Card className="mt-5 me-3 shadow" style={{ width: '16%', minWidth: '250px' }} key={item.id}>
